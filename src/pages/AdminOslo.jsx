...
function ArticleRow({ item, allItems, onSave, onDelete, onMove, onUpload, busy }) {
  ...
  const imgSrc = v.image_path ? articleImageUrl(v.image_path) : null
  ...
  return (
    <div className={`pe-row${v.published ? '' : ' pe-row--draft'}`}>
      <div className="pe-article-top">
        <div className="pe-img">
          {imgSrc ? (
            ...
            <div className="pe-img__empty">No image</div>
          )}
          <div className="pe-img__actions">
            ...
            >
              {uploading ? 'Uploading …' : v.image_path ? 'Replace' : 'Upload image'}
            </button>
            {v.image_path && (
              <button
                type="button"
        ...
        <div className="pe-article-fields">
          ...
          <Form.Control
            as="textarea"
            rows={2}
            placeholder="Short text (shown in the listing)"
            value={v.excerpt}
      ...
      <div className="pe-hint">
        <span>
          <code>## Heading</code>, <code>**bold**</code>, <code>- bullet</code>,{' '}
          <code>[text](/rsvp)</code>
        </span>
        <button
          type="button"
      ...
      <div className="pe-row-actions">
        ...
        <Button
          variant="primary"
          size="sm"
          disabled={busy || saving || uploading || !v.title.trim()}
          onClick={save}
        >
...
function AdminOsloInner() {
  ...
  const load = useCallback(async () => {
    const [{ data: arts, error: e1 }, { data: pc, error: e2 }] = await Promise.all(
      [
        supabase.rpc('admin_articles', { email, pass: password }),
        supabase
          .from('page_content')
          .select('value')
          .eq('key', 'oslo.unlocked')
          .maybeSingle(),
      ],
    )
    if (e1) {
      console.error('admin_articles failed:', e1)
      setError(true)
      return
    }
    if (e2) console.error('oslo.unlocked load failed:', e2)
    setError(false)
    setRows(arts || [])
  ...
  async function saveRow(item, v) {
    ...
    const { error } = await supabase.rpc('admin_article_save', {
      email,
      pass: password,
      p_id: item.id,
      p_title: v.title.trim(),
      p_excerpt: v.excerpt.trim(),
      p_body: v.body,
    ...
    setBusy(false)
    if (error) {
      console.error('admin_article_save failed:', error)
      window.alert('Could not save. Please try again.')
      return false
    }
    // Old image no longer referenced -> remove the file (best effort).
    if (item.image_path && item.image_path !== v.image_path) {
      supabase.storage.from('article-images').remove([item.image_path])
    }
    await load()
    return true
  }
  ...
  async function deleteRow(item) {
    ...
    setBusy(true)
    const { error } = await supabase.rpc('admin_article_delete', {
      email,
      pass: password,
      p_id: item.id,
    })
    setBusy(false)
    if (error) {
      console.error('admin_article_delete failed:', error)
  ...
  async function moveRow(item, dir) {
    ...
    setBusy(true)
    const { error } = await supabase.rpc('admin_articles_reorder', {
      email,
      pass: password,
    ...
    setBusy(false)
    if (error) {
      console.error('admin_articles_reorder failed:', error)
        ...
        <>
          <div className="pe-row">
            <Form.Check
              type="switch"
              id="oslo-unlock"
              label="Guests can open the “Vickie's Oslo” button"
              checked={unlocked}
              disabled={busy}
          ...
          {rows.length === 0 && newRows.length === 0 && (
            <p className="admin-status">No articles yet.</p>
          )}
